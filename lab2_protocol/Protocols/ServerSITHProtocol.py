...
class ServerSITHProtocol(SITHProtocol):
    ...
    def connection_made(self, transport):
        self.dbgPrint("Connection made at SITH layer on " + type(self.higherProtocol()).__name__)
            ...
            if isinstance(pkt, SITHPacket):  # judge if this packet is an instance of RIPPPacket
                if (pkt.Type == "HELLO") and (self.state == self.STATE_SERVER_HELLO):  #receive client hello pkt
                    ...
                    if self.verifyCerts(self.peerCerts):
                        self.dbgPrint("Server: SithHello received!")
                        ...
                        self.dbgPrint("Server: sending SithHello back to client... Current state: {!r}, random: {!r}"
                        ...
                        self.dbgPrint("Error: certificate verification failure.")
                        self.sendSithClose(1)

                elif (pkt.Type == "FINISH") and (self.state == self.STATE_SERVER_SITH_HANDSHAKE_DONE):

                    self.dbgPrint("Server: received handshake_finish packet from client")
                    ...
                    if not verify_result:
                        self.dbgPrint("Server: wrong signature of client!")
                        self.dbgPrint("Server is closing...")
                        self.sendSithClose(1)
                    else:
                    ...
                    self.dbgPrint("Error: wrong packet type " + pkt.DEFINITION_IDENTIFIER + ", current state "
                                     + self.STATE_DESC[self.state])
                    self.sendSithClose(1)

            else:
                self.dbgPrint("Wrong packet class type: {!r}".format(str(type(pkt))))
                self.sendSithClose(1)

    def connection_lost(self, exc):
        self.dbgPrint("Connection lost at SITH layer on " + type(self.higherProtocol()).__name__)
