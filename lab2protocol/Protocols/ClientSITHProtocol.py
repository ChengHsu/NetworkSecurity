...
class ClientSITHProtocol(SITHProtocol):
    ...
    def connection_made(self, transport):
        ...
        self.dbgPrint("Client: Sending Hello message, current state: {!r}, random: {!r}".format(self.STATE_DESC[self.state],
                                                                                      self.random))
            ...
            if isinstance(pkt, SITHPacket):   #judge if this packet is an instance of RIPPPacket
                ...
                if (pkt.Type == "HELLO") and (self.state == self.STATE_CLIENT_HELLO):   #receive server's hello pkt
                    ...
                    if self.verifyCerts(self.peerCerts):
                        self.dbgPrint("Client: received SithHello packet from server, current state: {!r}".format(
                    ...
                    self.dbgPrint("Client: received handshake_finish packet from server")
