...
class SITHProtocol(StackingProtocol):
    ...
    def __init__(self, higherProtocol=None):
        if higherProtocol:   #check if the connection has been made to higher layer
            self.dbgPrint("Initializing SITH layer on " + type(higherProtocol).__name__)
    ...
    def decrypt(self, nonce, data, aad):
        self.dbgPrint("Decrypting data at SITH layer on " + type(self.higherProtocol()).__name__)
        return self.decEngine.decrypt(nonce, data, aad)

    def encrypt(self, nonce, data, aad):
        self.dbgPrint("Encrypting data at SITH layer on " + type(self.higherProtocol()).__name__)
    ...
    def verifySithData(self, ciphertext, mac):
    ...
    def sendSithClose(self, error=None):
        if error:
            self.dbgPrint("Sending SithClose with error: ")
        else:
            self.dbgPrint("Sending SithClose...")
